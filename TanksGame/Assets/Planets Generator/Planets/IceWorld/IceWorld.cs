...
public class IceWorld : MonoBehaviour, IPlanet {
    ...
    public void SetColors(Color[] colors)
    {
        for (int i = 0; i < color_vars1.Length; i++)
        {
            m_PlanetUnder.SetColor(color_vars1[i], colors[i]);
        }
        for (int i = 0; i < color_vars2.Length; i++)
        {
            m_Lakes.SetColor(color_vars2[i], colors[i + color_vars1.Length]);
        }
        for (int i = 0; i < color_vars3.Length; i++)
        {
            m_Clouds.SetColor(color_vars3[i], colors[i + color_vars1.Length + color_vars2.Length]);
        }
    }
}
